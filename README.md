...
Speech recognition is local. Audio, correction history, settings, and logs remain on the computer. When recognition-improvement participation is enabled, saving a correction automatically sends sanitized correction JSON to the project endpoint. Microphone audio and local file paths are never included, and participation can be disabled in Settings. Audio is cleaned up according to the app's retention rules.
